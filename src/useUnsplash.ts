...
function useUnsplash({ apikey, id, options }: Props): [string, UnsplashObject] {
  const baseUrl: string = 'https://api.unsplash.com/photos/'
  const [image, setImage] = React.useState('')
  const [imageMeta, setImageMeta] = React.useState({})

  const applyOptions = (rawUrl: string): string => {
    const source = Object.keys(options).reduce(
